...
CHAIN_ID_T1 = 299792
ROUTER_SEPOLIA = Web3.to_checksum_address("0xAFdF5cb097D6FB2EB8B1FFbAB180e667458e18F4")
EXPLORER_URL_SEPOLIA = "https://sepolia.etherscan.io/tx/0x"
...
LANG = {
    'vi': {
        'title': 'GỬI ETH - SEPOLIA → T1 DEVNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'preparing_bridge': 'Đang chuẩn bị gửi...',
        'sending_bridge': 'Đang thực hiện gửi...',
        'success': 'Gửi thành công: {amount:.6f} ETH',
        ...
        'error': 'Lỗi',
        'connect_success': 'Thành công: Đã kết nối với Sepolia',
    ...
    'en': {
        ...
        'preparing_bridge': 'Preparing deposit...',
        'sending_bridge': 'Sending deposit...',
        'success': 'Deposit successful: {amount:.6f} ETH',
        ...
        'error': 'Error',
        'connect_success': 'Success: Connected to Sepolia',
        ...
        'amount_prompt': 'Enter ETH amount to deposit',
        'invalid_amount': 'Invalid amount, please enter a number greater than 0',
        'times_prompt': 'Enter number of deposits',
...
async def deposit(w3: Web3, private_key: str, wallet_index: int, amount: int, deposit_times: int, language: str = 'en'):
    account = Account.from_key(private_key)
    sender_address = account.address
    contract = w3.eth.contract(address=ROUTER_SEPOLIA, abi=BRIDGE_ABI)
    successful_deposits = 0
    nonce = w3.eth.get_transaction_count(sender_address, 'pending')
    extra_fee = int(0.000000000000168 * 10**18)  # 0.000000000000168 ETH

    for i in range(deposit_times):
        print_border(f"Deposit {i+1}/{deposit_times}: {amount / 10**18:.6f} ETH", Fore.YELLOW)
        ...
        try:
            ...
            tx_link = f"{EXPLORER_URL_SEPOLIA}{tx_hash.hex()}"
...
async def run_deposit(language: str = 'en'):
    print()
    ...
    for i, (profile_num, private_key) in enumerate(private_keys, 1):
        ...
        sepolia_balance = check_balance(w3_sepolia, account.address, language)
        print()
        while True:
            print(f"{Fore.CYAN}{LANG[language]['amount_prompt']} {Fore.YELLOW}(Max: {sepolia_balance:.4f} ETH){Style.RESET_ALL}")
            try:
                amount_input = float(input(f"{Fore.GREEN}  > {Style.RESET_ALL}"))
                if amount_input > 0 and amount_input <= sepolia_balance:
        ...
        while True:
            ...
            try:
                deposit_times = int(input(f"{Fore.GREEN}  > {Style.RESET_ALL}"))
                if deposit_times > 0:
                    break
        ...
        print()
        deposits = await deposit(w3_sepolia, private_key, profile_num, amount, deposit_times, language)
        successful_deposits += deposits
        total_deposits += deposit_times
